...
{
    [ObjectSystem]
    public class GamerTurnComponentUpdateSystem : UpdateSystem<GamerTurnComponent>
    {
        public override void Update(GamerTurnComponent self)
        {
            self.Update();
        }
    }

    public class GamerTurnComponent : Component
    {
        // turn
        ...
        private void UpdateTurn()
        {
            ...
            this.GetParent<Gamer>().Rotation = v;
        }
        ...
        {
            Vector3 nexpos = this.GetParent<Gamer>().GameObject.transform.position + dir;
        ...
        {
            Quaternion quaternion = PositionHelper.GetVector3ToQuaternion(this.GetParent<Gamer>().Position, target);

            this.To = quaternion;
            this.From = this.GetParent<Gamer>().Rotation;
            this.t = 0;
        ...
        {
            this.GetParent<Gamer>().Rotation = quaternion;
        }

        public void TurnImmediately(Vector3 target)
        {
            Vector3 nowPos = this.GetParent<Gamer>().Position;
            if (nowPos == target)
            {
                return;
            }
